...
import {
  ...
  Briefcase, Lock, Eye, EyeOff, CheckCircle, Shield, Calendar
} from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const handleChangePassword = async () => {
    ...
    if (pwd.newPassword !== pwd.confirmPassword) { toast.error('Las contraseñas no coinciden'); return }
    setSavingPwd(true)
    try {
      const res  = await fetch('/api/profile', {
        method: 'PATCH',
  ...
  return (
    <div className="space-y-6 w-full max-w-2xl">
      ...
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">Gestiona tu información personal y seguridad</p>
      </div>
      ...
      <div className="flex gap-1 p-1 bg-gray-900 rounded-xl border border-gray-800">
        {([['info', 'Información personal', User], ['security', 'Seguridad', Lock]] as const).map(([id, label, Icon]) => (
          <button
            key={id}
        ...
        <div className="glass rounded-2xl p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              { key: 'phone',    label: 'Teléfono',        icon: Phone,     type: 'tel',   placeholder: '+52 800 000 0000' },
              { key: 'position', label: 'Cargo',           icon: Briefcase, type: 'text',  placeholder: 'Tu cargo' },
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            {[
              { label: 'Miembro desde', value: profile ? formatDate(profile.createdAt) : '—', icon: Calendar },
      ...
      {activeTab === 'security' && (
        <div className="glass rounded-2xl p-6 space-y-4">
          ...
          {([ ['currentPassword', 'Contraseña actual', 'current'], ['newPassword', 'Nueva contraseña', 'new'], ['confirmPassword', 'Confirmar nueva contraseña', 'confirm'] ] as const).map(([key, label, showKey]) => (
            <div key={key}>
          ...
          {pwd.newPassword && (
            <div className="space-y-1.5">
              <div className="flex gap-1">
                {[1,2,3,4].map(i => {
                  const strength = [pwd.newPassword.length >= 6, /[A-Z]/.test(pwd.newPassword), /[0-9]/.test(pwd.newPassword), /[^A-Za-z0-9]/.test(pwd.newPassword)].filter(Boolean).length
                  return <div key={i} className={cn('h-1 flex-1 rounded-full transition-all', i <= strength ? ['bg-red-400','bg-amber-400','bg-yellow-400','bg-emerald-400'][strength - 1] : 'bg-gray-700')} />
                })}
              </div>
              <p className="text-[10px] text-gray-500">
                {[,'Muy débil','Débil','Aceptable','Segura'][[pwd.newPassword.length >= 6, /[A-Z]/.test(pwd.newPassword), /[0-9]/.test(pwd.newPassword), /[^A-Za-z0-9]/.test(pwd.newPassword)].filter(Boolean).length]}
              </p>
            </div>
          )}
