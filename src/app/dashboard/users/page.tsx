...
export default function UsersPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (u: User) => (
        <span
          className={`text-xs font-medium px-2.5 py-1 rounded-full ${u.isActive ? "text-emerald-400 bg-emerald-400/10" : "text-red-400 bg-red-400/10"}`}
        >
    ...
    },
    {
      ...
      render: (u: User) => (
        <div className="flex items-center gap-1.5 justify-end">
          ...
          <button
            onClick={() => openEdit(u)}
            className="w-8 h-8 rounded-lg bg-gray-800 hover:bg-gray-700 flex items-center justify-center text-gray-400 hover:text-white"
          >
            <Pencil className="w-3.5 h-3.5" />
          </button>
          <button
            onClick={() => toggleActive(u)}
            className={`w-8 h-8 rounded-lg flex items-center justify-center ${u.isActive ? "bg-red-500/10 hover:bg-red-500/20 text-red-400" : "bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400"}`}
          >
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <Modal
        ...
        title={editUser ? "Editar Empleado" : "Nuevo Empleado"}
        size="lg"
      >
