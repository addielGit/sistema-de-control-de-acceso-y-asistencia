...
export default function SettingsPage() {
  ...
  function toggleDay(day: number) {
    ...
    if (next.length === 0)
      return toast.error("Debe haber al menos un día laboral");
  ...
  function addHoliday() {
    ...
    if (config.holidays.includes(newHoliday))
      return toast.error("Esa fecha ya está registrada");
    const entry = holidayLabel.trim()
      ? `${newHoliday}|${holidayLabel.trim()}`
      : newHoliday;
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <p className="text-sm text-gray-400">
          Selecciona los días que se consideran hábiles para el cálculo de
          asistencia.
        </p>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(e) =>
                  update({ checkInTolerance: parseInt(e.target.value) })
                }
                className="flex-1 accent-blue-500"
              />
            </div>
            <p className="text-xs text-gray-500 mt-1">
              El empleado puede llegar hasta {config.checkInTolerance} min tarde
              sin registrar retardo.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(e) =>
                  update({ checkOutTolerance: parseInt(e.target.value) })
                }
                className="flex-1 accent-blue-500"
              />
            </div>
            <p className="text-xs text-gray-500 mt-1">
              El empleado puede salir hasta {config.checkOutTolerance} min antes
              sin penalización.
            </p>
          </div>
        </div>
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <p className="text-sm text-gray-400">
          Los días festivos no contarán como ausencias aunque el empleado no
          registre asistencia.
        </p>
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-lg bg-red-500/0 group-hover:bg-red-500/10 flex items-center justify-center text-gray-600 group-hover:text-red-400 transition-all"
                  >
        ...
        {config.holidays.length > 0 && (
          <p className="text-xs text-gray-500">
            {config.holidays.length} día
            {config.holidays.length !== 1 ? "s" : ""} festivo
            {config.holidays.length !== 1 ? "s" : ""} configurado
            {config.holidays.length !== 1 ? "s" : ""}
          </p>
        )}
      </div>
