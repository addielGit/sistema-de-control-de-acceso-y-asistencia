...
const checkSchema = z.object({
  ...
  // Client timezone offset in minutes (new Date().getTimezoneOffset())
...
export async function POST(req: NextRequest) {
  ...
  if (alreadyIn && alreadyOut)
    return NextResponse.json({ error: 'Ya se registró entrada y salida hoy.', code: 'ALREADY_DONE' }, { status: 409 })
  ...
  let attendance, action: 'ENTRY' | 'EXIT', message: string
  ...
  if (checkType === 'entry') {
    ...
    attendance = await prisma.attendance.upsert({
      ...
      create: { userId, date: today, checkIn: now, status, lateMinutes, notes: reason },
    ...
    message = isAdminOverride
      ...
      : lateMinutes > 0 ? `Entrada registrada con ${lateMinutes} min de retardo` : 'Entrada registrada exitosamente'
    ...
    if (isAdminOverride) {
      ...
      if (lateMinutes >= 5) await notifyAdmins('Alerta de retardo', `${user.name} registró entrada con ${lateMinutes} min de retardo`, 'WARNING')
    }
  } else {
  ...
  await prisma.accessLog.create({ data: { userId, action, method, ipAddress, location } })

  if (isAdminOverride) {
    await prisma.auditLog.create({
      data: { actorId: session.user.id, userId, action: `ADMIN_MARK_${action}`, entity: 'Attendance', entityId: attendance.id, newData: { time: now, reason } },
    })
  }
