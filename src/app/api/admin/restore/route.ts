...
export async function POST(req: NextRequest) {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (!file || typeof file === "string") {
        return NextResponse.json(
          { error: "No se encontró el archivo en el formulario" },
          { status: 400 },
        );
      }
  ...
  if (!backup?.meta?.version || !backup?.data) {
    return NextResponse.json(
      { error: "El archivo no es un backup válido de AccessFlow" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (workConfig?.length) {
      await prisma.workConfig.createMany({
        data: workConfig,
        skipDuplicates: true,
      });
    }
    ...
    return NextResponse.json(
      { error: `Error al restaurar: ${err.message}` },
      { status: 500 },
    );
  }
}
