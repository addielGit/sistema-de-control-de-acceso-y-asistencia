  ...
  return (
    <Modal open={open} onClose={onClose} title="Marcaje manual" size="md">
      ...
      <div className="flex items-center gap-3 p-3 bg-gray-900/60 rounded-xl border border-gray-800 mb-5">
        <div className="w-10 h-10 rounded-xl bg-blue-500/20 border border-blue-500/30 flex items-center justify-center text-blue-400 font-semibold text-sm shrink-0">
          {user.name[0]}
        </div>
        <div>
      ...
      {loadingStatus ? (
        ...
        <div className="flex flex-col items-center py-6 gap-3 text-center">
          ...
          <button
            onClick={onClose}
            className="mt-2 px-4 py-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-sm text-gray-300 transition-all"
          >
            Cerrar
          </button>
        </div>
          ...
          <div className="flex gap-4 text-xs text-gray-400">
            <span>
              ...
              <span className="text-white font-mono">
                {formatTime(new Date(todayStatus!.checkIn!))}
              </span>
            </span>
            <span>
              Salida:{" "}
              <span className="text-white font-mono">
                {formatTime(new Date(todayStatus!.checkOut!))}
              </span>
            </span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-gray-900/60 rounded-xl p-3 text-center border border-gray-800">
              ...
              <p
                className={cn(
                  ...
                  checkedIn ? "text-emerald-400" : "text-gray-600",
                )}
              >
                {checkedIn
                  ? formatTime(new Date(todayStatus!.checkIn!))
            ...
            <div className="bg-gray-900/60 rounded-xl p-3 text-center border border-gray-800">
              ...
              <p
                className={cn(
                  "text-sm font-mono font-semibold",
                  checkedOut ? "text-orange-400" : "text-gray-600",
                )}
              >
                {checkedOut
                  ? formatTime(new Date(todayStatus!.checkOut!))
          ...
          >
            ...
            <p
              className={cn(
                "text-xs font-medium",
                actionType === "ENTRY" ? "text-emerald-300" : "text-orange-300",
              )}
            >
              {actionType === "ENTRY"
          ...
          <div className="flex items-start gap-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            ...
            <p className="text-xs text-amber-300">
              Esta acción quedará registrada en el log de auditoría con tu
              usuario, la hora y la razón indicada.
            </p>
          </div>
