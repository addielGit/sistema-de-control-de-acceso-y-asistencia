...
import {
  Menu, Bell, X, Check, CheckCheck, Trash2, Loader2,
...
function NotificationPanel() {
  ...
  const panel = (
    <div id="notif-portal" style={pStyle} className="flex flex-col max-h-[520px] bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl shadow-black/60">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800 shrink-0">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-white">Notificaciones</h3>
          {unread > 0 && <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-500/20 text-blue-400 border border-blue-500/30">{unread} nuevas</span>}
        </div>
        <div className="flex items-center gap-1">
          {unread > 0 && (
            <button onClick={markAll} disabled={acting === 'all'} className="flex items-center justify-center text-gray-400 hover:text-white transition-all p-1">
              {acting === 'all' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCheck className="w-3.5 h-3.5" />}
            </button>
          )}
          {readList.length > 0 && (
            <button onClick={clearRead} disabled={acting === 'clear'} className="flex items-center justify-center text-gray-400 hover:text-red-400 transition-all p-1">
              {acting === 'clear' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
            </button>
          )}
          <button onClick={() => setOpen(false)} className="flex items-center justify-center text-gray-400 hover:text-white transition-all p-1">
      ...
      <div className="overflow-y-auto flex-1">
        {loading ? (
          ...
          <div className="py-2">
            {unreadList.length > 0 && <><p className="px-4 py-1.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Nuevas</p>{unreadList.map(n => <NRow key={n.id} n={n} onRead={markOne} onDel={del} acting={acting} />)}</>}
            {readList.length > 0 && <><p className="px-4 py-1.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wider mt-1">Anteriores</p>{readList.map(n => <NRow key={n.id} n={n} onRead={markOne} onDel={del} acting={acting} />)}</>}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <>
      <button ref={btnRef} onClick={() => { const next = !open; setOpen(next); if (next) load() }}
...
function NRow({ n, onRead, onDel, acting }: { n: Notif; onRead: (id: string) => void; onDel: (id: string) => void; acting: string | null }) {
  const cfg = NOTIF_CFG[n.type] || NOTIF_CFG.INFO
  const Icon = cfg.icon
  const ago = formatDistanceToNow(new Date(n.createdAt), { addSuffix: true, locale: es })
  return (
    <div onClick={() => !n.isRead && onRead(n.id)} className={cn('group flex gap-3 px-4 py-3 transition-colors cursor-pointer', n.isRead ? 'hover:bg-gray-800/40' : 'bg-blue-500/5 hover:bg-blue-500/10 border-l-2 border-l-blue-500')}>
