...
import { Clock, CheckCircle, Loader2, LogIn, LogOut, AlertTriangle, Ban, CalendarX } from 'lucide-react'
...
export function EmployeeDashboard({ userId }: { userId: string }) {
  ...
  const refresh = () => {
    ...
    fetch(`/api/attendance/check?userId=${userId}`)
      .then(r => r.json())
      .then(d => { setToday(d.attendance); setSchedule(d.schedule) })
  ...
  return (
    <div className="space-y-6 w-full max-w-2xl">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1 capitalize">{formatDate(now, "EEEE, d 'de' MMMM yyyy")}</p>
      </div>
      ...
      <div className="glass rounded-2xl p-8 text-center relative overflow-hidden">
        ...
        <div className={cn('text-6xl font-mono font-bold tracking-tight mb-1 transition-colors', getClockColor())}>
          {formatTime(now)}
        </div>
          ...
          <div className="flex items-center justify-center gap-4 mb-6 text-xs text-gray-500">
            {!checkedIn && (
              <span className="flex items-center gap-1">
                ...
                Entrada hasta {new Date(schedule.checkInDeadline).toTimeString().slice(0,5)}
              </span>
            )}
            {checkedIn && !checkedOut && (
              <span className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-orange-400" />
                Salida desde {new Date(schedule.checkOutStart).toTimeString().slice(0,5)}
              </span>
            )}
          </div>
        )}
        ...
        {loading ? (
          <div className="flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-blue-400" /></div>
        ) : done ? (
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-left">
              ...
              <div>
                <p className="text-sm font-medium text-red-300">Marcaje no disponible</p>
                <p className="text-xs text-red-400/80 mt-1">{btnState.reason}</p>
                <p className="text-xs text-gray-500 mt-2">Contacta a tu administrador para un marcaje manual.</p>
              </div>
            </div>
          </div>
        ) : (
      ...
      <div className="glass rounded-2xl p-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Estado',   value: today ? <StatusBadge status={today.status} /> : <span className="text-gray-500 text-xs">Sin registrar</span> },
            { label: 'Entrada',  value: today?.checkIn  ? formatTime(new Date(today.checkIn))  : '--:--' },
            { label: 'Salida',   value: today?.checkOut ? formatTime(new Date(today.checkOut)) : '--:--' },
          ].map(({ label, value }) => (
        ...
        {(today?.lateMinutes ?? 0) > 0 && (
          <div className="mt-4 flex items-center gap-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            ...
            <p className="text-xs text-amber-300">Retardo registrado: {today!.lateMinutes} minutos</p>
          </div>
        )}
      </div>
      ...
      {schedule && (
        <div className="glass rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-white mb-3">Horario de hoy</h3>
          <div className="grid grid-cols-2 gap-3 text-center">
            {[
              { label: 'Entrada programada', value: schedule.checkInTime,  color: 'text-emerald-400' },
              { label: 'Salida programada',  value: schedule.checkOutTime, color: 'text-orange-400' },
              { label: 'Límite de entrada',  value: new Date(schedule.checkInDeadline).toTimeString().slice(0,5), color: 'text-amber-400' },
              { label: 'Salida mínima',      value: new Date(schedule.checkOutStart).toTimeString().slice(0,5),   color: 'text-blue-400'  },
          ...
          {!schedule.isWorkDay && (
            <div className="mt-3 flex items-center gap-2 p-3 bg-gray-700/20 border border-gray-700 rounded-xl">
              ...
              <p className="text-xs text-gray-400">Hoy es día no laboral — no se registra asistencia</p>
            </div>
          )}
        </div>
      )}
